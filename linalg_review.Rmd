...
Let's consider a situation where we want to find the gradient of a function wtih respect to a matrix, namely for $A \in \mathbb{R}^{n \times n}$, we want to find $\nabla_A |A|$. Recall from our discussion of determinants (section 3.10) that
$$
|A| = \sum_{i=1}^n(-1)^{i+j}A_{ij} M_{ij} \quad (\text{for any } j \in 1,\dots,n, \text{ and }M_{ij}=|A_{\backslash i, \backslash j}|)
$$
so
$