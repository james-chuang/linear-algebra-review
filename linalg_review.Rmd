...
The product of two matrices $A \in \mathbf{R}^{m \times n}$ and $B \in \mathbf{R}^{n \times p}$ is the matrix
$$
...
A square matrix $A \in \mathbf{R}^{n \times n}$ is *symmetric* if $A=A^T$. It is anti-symmetric if $A=-A^T$.

For any matrix $A \in \mathbf{R}^{n \times n}$, the matrix $A + A^T$ is symmetric:
$$
\begin{align}
A + A^T & = \begin{bmatrix}
            A_{1,1} & A_{1,2} & \cdots  & A_{1,n} \\
...
From these properties, it follows that any square matrix $A \in \mathbf{R}^{n \times n}$ can be represented as a sum of a symmetric matrix and an anti-symmetric matrix:
$$
A = \frac{1}{2}\left(A+A^T\right)+ \frac{1}{2}\left(A-A^T\right)
$$

Symmetric matrices have nice properties and occur often, particularly when they are generated by a function of two arguments that does not depend on the order of the arguments (e.g. a distance measure between two points). The set of all symmetric matrices of size $n$ can be denoted as $\mathbf{S}^n$.

## **3.4 the trace **
The ***trace*** of a square matrix $A \in \mathbf{R}^{n \times n}$, denoted $\text{tr}(A)$ or $\text{tr}A$, is the sum of diagonal elements n the matrix:
